...
from users.hp.culiarity.pyculiarity.date_utils import format_timestamp, get_gran, date_format, datetimes_from_ts
from users.hp.culiarity.pyculiarity.detect_anoms import detect_anoms
...
#    if not isinstance(xlabel, basestring):原版本
    if not isinstance(xlabel, str):
        raise ValueError("xlabel must be a string")

#    if not isinstance(ylabel, basestring):原版本
...
#    if title and not isinstance(title, basestring):
    ...
    if gran == "day":
        num_days_per_line = 7
#        if isinstance(only_last, basestring) and only_last == 'hr':yuanbanben
